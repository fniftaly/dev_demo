...
//    $val_camp = $rep->getUserTotalCampaign($accounts_id[$uid]); 
    ...
    /* mo mts comes from from mt_for_mo_request table and we need to add it to the total sms*/
    
    $val_totalmrk = $rep->getTotalMarketingMt($accounts_id[$uid],'ym');
    
    $totalsms = $val_kwd+$val_totalmrk;
    
    $rep->activateUserproperty($accounts_id[$uid], $mrkinfo[6], $totalsms);
