...
// Log file for today
$logfile = $logpath . date('Ymd') . '-dailyautosms';
...
if (is_array($sendReportsTo)) {
    if (!empty($sendReportsTo)) {
        foreach ($sendReportsTo as $ars => $user) {
            ...
            if ($user['continent'] !=3) {
                if ($user['edituser'] != '0') {
                    if($user['continent'] ==0){ $user['continent'] = 3;}
            ...
            if($sendiftrue){
              $mail->setBodyText('Daily report....\n');
            }else{
                $mail->setBodyText('No members are selected....\n');
            }
            // Send it on
