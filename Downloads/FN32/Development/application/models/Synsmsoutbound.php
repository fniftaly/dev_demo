<?php
class Application_Model_Smsoutbound extends Application_Model_Synsmsabstract {
	/**
...
//                exit;
		...
		foreach ($recipients as $_r) {
			$sql = "CALL message_log_queue($gatewaymessageid, '$_r', {$this->message->reportingkey1}, '{$this->message->reportingkey2}', '$this->_sendtime', '$timezone',$createuser)";
...
//			 $uri =$this->_apiUrl.'?user='.$this->_username.'&pass='.$this->_password.'&smsfrom='.$this->_shortCode.'&smsto='.implode('&recipient=', $sendTo).'&smsmsg='.$sendMessage;
			// Now handle sending, start by opening a cURL handle
			$ch = curl_init($this->_apiUrl);
                                            curl_setopt ($ch, CURLOPT_POST, 1);
		               curl_setopt ($ch, CURLOPT_POSTFIELDS, "user=$this->_username&pass=$this->_password&smsfrom=$this->_shortCode&smsto=$sendToRecip&smsmsg=$sendMessage");
	...
	public function send() {
		...
		if ($this->message && $this->message instanceof Application_Model_Message && $this->message->id && $this->message->body && $this->message->recipients && $this->message->reportingkey1 && $this->message->reportingkey2) {
			...
			while ($recipientsFetched < $recipientsCount) {
				...
				$sendToRecip = implode('&recipient=', $sendTo);
				// Build up the request querystring
//				$uri = $this->_apiUrl . '?reply_to=' . $this->_shortCode . '&recipient=' . implode('&recipient=', $sendTo) . '&body=' . $sendMessage . '&reporting_key1=' . $this->message->reportingkey1 . '&reporting_key2=' . $this->message->reportingkey2;
//				 $uri =$this->_apiUrl.'user='.$this->_username.'&pass='.$this->_password.'&smsfrom='.$this->_shortCode.'&smsto='.implode('&recipient=', $sendTo).'&smsmsg='.$sendMessage;
				// Now handle sending, start by opening a cURL handle
...
//				curl_setopt($ch, CURLOPT_USERPWD, $this->_username . ':' . $this->_password);
				...
				// Now lets inspect it and see if we have what we need
				$response = simplexml_load_string($rs);
//                                                          if($this->_shortCode ==87365){
//				  $this->synsend($this->_shortCode,$sendToRecip,$sendMessage); // gateway to Syniverse server
//                                                          }else{
//                                                              $this->_shortCode =28776;
//                                                              $this->synsend($this->_shortCode,$sendToRecip,$sendMessage);
//                                                          }
                                                          // Type cast the response code and description for use
				$code = intval($response->trackingid);
				$description = "$response->description";
...
//                              $uri = $apiUrl."?user=$username&pass=$password&smsfrom=$shortCode&smsto=$sendTo&smsmsg=$sendMessage";
	 ...
	 * @return boolean True on success, false on failure
	 */
	public function sendNow_stop($message = '', Array $recipients = array()) {
		// Validate our username and password
		if (empty($this->_username) || empty($this->_password)) {
...
//                                          $uri =$this->_apiUrl.'?user='.$this->_username.'&pass='.$this->_password.'&smsfrom='.$this->_shortCode.'&smsto='.implode('&recipient=', $sendTo).'&smsmsg='.$sendMessage;
//			$uri = $this->_apiUrl . '?reply_to=' . $this->_shortCode . '&recipient=' . implode('&recipient=', $sendTo) . '&body=' . $sendMessage . '&reporting_key1=' . $this->message->reportingkey1 . '&reporting_key2=' . $this->message->reportingkey2;
//			 curl_setopt ($ch, CURLOPT_POSTFIELDS, "user=$this->_username &pass=$this->_password&smsfrom=$this->_shortCode&smsto=implode('&recipient=', $sendTo)&smsmsg=$sendMessage");
			// Now handle sending, start by opening a cURL handle
			$ch = curl_init($this->_apiUrl);
                                            curl_setopt ($ch, CURLOPT_POST, 1);
		               curl_setopt ($ch, CURLOPT_POSTFIELDS, "user=$this->_username&pass=$this->_password&smsfrom=$this->_shortCode&smsto=$sendToRecip&smsmsg=$sendMessage");
...
//                 if(isset($this->message->createuser)){
//                  $user          = isset($this->message->createuser) ? $this->message->createuser : 1;
//                 }else{
//                   $user          = isset($this->message->user) ? $this->message->user->getId() : 1;
//                 }
...
//        $sql = sprintf("CALL message_log('$campaignid',$scheduleid,$sponsorid,$linkid,$locationid,'%s','%s',$user, $reportingkey1,'$reportingkey2',$keywordid,$folderid,'%s','%s')",
//                    $this->escape($this->message->body),
//                    $this->escape($replybody),
//                    $this->escape($stdfolders),
//                    $this->escape($description)
//                );
